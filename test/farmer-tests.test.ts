...
describe("Polygon SushiFarmer Tests", function () {
    ...
    const addLiquidityAndDeposit = async (
        ...
        const [initialStaked] = await MiniChef.userInfo(
            pid,
            SushiFarmer.address
        );
        ...
        const [independentTokenName, dependentTokenName] =
            getUnderlyingTokenNames(
        ...
        const txn = await SushiFarmer.connect(
            WhaleSigner
        ...
        const decodedLogData = iface.decodeEventLog(
            "LPDeposited",
            lPDepositedData
        );
    ...
    before(async () => {
        ...
        independentTokenInfo = {
            ...
            amount: ethers.utils
                .parseUnits("1", independentDecimals)
                .toString(),
        ...
        dependentTokenInfo = {
            ...
            amount: ethers.utils
                .parseUnits("2500", dependentDecimals)
                .toString(),
    ...
    it("Should be able to remove LP position from contract and withdraw.", async function () {
        ...
        const lpBalanceAfterWithdraw = await Whale.V2Pair.balanceOf(
            Whale.address
        );
    ...
    it.only("Should be able to autocompound LP position.", async function () {
        ...
        const latestBlock =
            (await sushi.blocks.latestBlock({ chainId: 137 })) - 2;
        ...
        const [staked] = await Whale.MiniChef.userInfo(
            pid,
            SushiFarmer.address
        );
        ...
        console.log(
            "********** Autocompounded LP Position in MiniChef **********"
        );
