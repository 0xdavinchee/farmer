...
export const setup = async (data: ISetupProps) => {
    ...
    const contracts = {
        ...
        // reward tokens
        RewardTokenA: await ethers.getContractAt<IERC20>(
            ERC20ABI,
            rewardTokenA
        ),
        RewardTokenB: await ethers.getContractAt<IERC20>(
            ERC20ABI,
            rewardTokenB
        ),
...
export const getRewardToTokenPaths = (
    ...
    for (let i = 0; i < rewardTokenData.length; i++) {
        ...
        for (let j = 0; j < tokenAddresses.length; j++) {
            ...
            const path = rewardTokenToTokenTrade[0].route.path.map(
                (x) => x.address
            );
            object.tokenAPath.length === 0
...
export const getLPTokenAmounts = async (
    ...
    let amount0 =
        ...
        (await user.IndependentToken.balanceOf(farmer));
    ...
    if (amount1 > amount1Balance) {
        ...
        console.log(
            "Attempting with dependent token as the independent token."
        );
...
export const printTokensBalance = async (whale: IUser, user: string) => {
    const independentTokenBalance = await whale.IndependentToken.balanceOf(
        user
    );
